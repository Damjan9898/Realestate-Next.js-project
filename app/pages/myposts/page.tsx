...
const MyPostsBody: React.FC<MyPostsBodyProps> = ({
    currentUser
})=>{
    //User id
      ...
      return (
        <div>
            <div className="py-[150px] xl:w-[1000px] m-auto w-[90%]">
              ...
              {allposts?.length !==0 ? (
                <>
                  {allposts.filter(post => post.userId == userId).map((post) => (

                      <OnePost isadmin redirect={`http://localhost:3000/post/${post.id}`} id={post.id} title={post.title} price={post.price} image={post.imageUrls[0]} surface={post.surface} location={post.location}/>
                  ))}
                </>
                ) : (
